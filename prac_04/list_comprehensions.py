...
for i in range(len(full_names)):
    lowercase_full_names.append(full_names[i].lower())
...
for i in range(len(almost_numbers)):
    numbers.append(int(almost_numbers[i]))
print(numbers)
...
for i in range(len(numbers)):
    if numbers[i] > 9:
        numbers_greater_than_9.append(numbers[i])
...
names_longer_than_11 = []
for i in range(len(full_names)):
    if len(full_names[i]) > 11:
        last_name = full_names[i].split(" ")
        names_longer_than_11.append(last_name[1])
